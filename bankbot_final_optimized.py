...
st.markdown("""
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .block-container { padding: 2rem 3rem; max-width: 100%; background: #FFFFFF; }
    p, span, div, label, h1, h2, h3, h4, h5, h6 { color: #000000 !important; }
    
    .login-container {
        max-width: 480px; margin: 3rem auto; background: #FFFFFF;
        border-radius: 24px; padding: 3rem;
        box-shadow: 0 8px 32px rgba(0,0,0,0.1); border: 2px solid #e5e7eb;
    ...
    .login-logo {
        ...
        font-size: 40px; margin: 0 auto 1.5rem auto;
        box-shadow: 0 8px 24px rgba(59, 130, 246, 0.3);
    }
    .login-title { font-size: 32px; font-weight: 700; color: #000000 !important; margin-bottom: 0.5rem; font-family: 'Space Grotesk', sans-serif; }
    .login-subtitle { font-size: 16px; color: #000000 !important; margin-bottom: 0.5rem; }
    .login-batch {
        ...
        font-size: 13px; font-weight: 600; margin-top: 0.5rem;
    }
    .demo-credentials { background: #FFFFFF; border: 2px solid #e5e7eb; border-radius: 12px; padding: 1rem; margin-top: 1.5rem; font-size: 13px; }
    .demo-credentials strong { color: #000000 !important; display: block; margin-bottom: 0.5rem; }
    .demo-credentials p { color: #000000 !important; margin: 0.25rem 0; font-family: 'Courier New', monospace; }
    .demo-credentials code { background: #f3f4f6; padding: 0.2rem 0.5rem; border-radius: 4px; color: #000000 !important; }
    
    section[data-testid="stSidebar"] { background: #FFFFFF !important; box-shadow: 4px 0 20px rgba(0,0,0,0.08); border-right: 2px solid #e5e7eb; }
    section[data-testid="stSidebar"] > div { padding-top: 2rem; background: #FFFFFF; }
    
    .chat-hero { text-align: center; padding: 2rem 0 3rem 0; background: #FFFFFF; }
    .chat-hero h1 { font-size: 36px; font-weight: 700; color: #000000 !important; margin-bottom: 0.5rem; font-family: 'Space Grotesk', sans-serif; }
    .chat-hero p { font-size: 18px; color: #000000 !important; }
    
    .robot-container { display: flex; justify-content: center; align-items: center; gap: 3rem; margin: 3rem 0; padding: 2rem; background: #FFFFFF; }
    .robot-visual { position: relative; width: 260px; height: 260px; }
    .robot-glow { position: absolute; inset: 0; background: radial-gradient(circle, rgba(59, 130, 246, 0.15) 0%, transparent 70%); animation: pulse 3s ease-in-out infinite; }
    @keyframes pulse { 0%, 100% { transform: scale(1); opacity: 0.5; } 50% { transform: scale(1.1); opacity: 0.8; } }
    .robot-emoji { position: relative; font-size: 160px; filter: drop-shadow(0 10px 30px rgba(59, 130, 246, 0.2)); animation: float 4s ease-in-out infinite; }
    @keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-20px); } }
    
    .welcome-bubble {
        background: #FFFFFF; padding: 2rem 2.5rem; border-radius: 24px;
        box-shadow: 0 8px 32px rgba(0,0,0,0.1); border: 2px solid #e5e7eb;
        position: relative; max-width: 420px; animation: slideIn 0.6s ease-out;
    }
    @keyframes slideIn { from { opacity: 0; transform: translateX(30px); } to { opacity: 1; transform: translateX(0); } }
    .welcome-bubble::before {
        content: ''; position: absolute; left: -12px; top: 50px;
        width: 0; height: 0; border-top: 12px solid transparent;
        border-bottom: 12px solid transparent; border-right: 12px solid #FFFFFF;
        filter: drop-shadow(-2px 0 0 #e5e7eb);
    }
    .welcome-bubble h3 { font-size: 22px; font-weight: 600; color: #000000 !important; margin-bottom: 0.5rem; }
    .welcome-bubble p { font-size: 18px; color: #000000 !important; line-height: 1.6; }
    
    .chat-message { margin: 1.5rem 0; padding: 1.25rem 1.5rem; border-radius: 16px; animation: fadeInUp 0.4s ease-out; background: #FFFFFF; border: 2px solid #e5e7eb; }
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    .chat-message.user { background: #FFFFFF; margin-left: 20%; border: 2px solid #3b82f6; box-shadow: 0 2px 12px rgba(59, 130, 246, 0.1); }
    .chat-message.bot { background: #FFFFFF; margin-right: 20%; border: 2px solid #e5e7eb; box-shadow: 0 2px 12px rgba(0,0,0,0.05); }
    .message-header { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem; }
    .message-avatar { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 18px; }
    .message-avatar.user { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); }
    .message-avatar.bot { background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); }
    .message-name { font-weight: 600; color: #000000 !important; font-size: 14px; }
    .message-time { color: #6b7280 !important; font-size: 12px; margin-left: auto; }
    .message-content { color: #000000 !important; line-height: 1.6; font-size: 15px; }
    
    .stButton button {
        background: #FFFFFF !important; color: #000000 !important; border: 2px solid #e5e7eb !important;
        border-radius: 14px !important; padding: 0.875rem 1.5rem !important; font-weight: 600 !important;
        font-size: 15px !important; transition: all 0.3s !important; box-shadow: 0 2px 8px rgba(0,0,0,0.05) !important;
        font-family: 'Inter', sans-serif !important; width: 100%;
    }
    .stButton button:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 20px rgba(59, 130, 246, 0.15) !important; border-color: #3b82f6 !important; background: #f9fafb !important; }
    
    .stTextInput input { border: 2px solid #e5e7eb !important; border-radius: 12px !important; padding: 0.875rem 1rem !important; font-size: 15px !important; color: #000000 !important; background: #FFFFFF !important; transition: all 0.3s !important; }
    .stTextInput input:focus { border-color: #3b82f6 !important; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1) !important; outline: none !important; }
    .stTextInput input::placeholder { color: #6b7280 !important; }
    .stTextInput label { color: #000000 !important; font-weight: 600 !important; font-size: 14px !important; margin-bottom: 0.5rem !important; }
    
    .stTextArea textarea { border: 2px solid #e5e7eb !important; border-radius: 12px !important; padding: 0.875rem 1rem !important; font-size: 15px !important; color: #000000 !important; background: #FFFFFF !important; transition: all 0.3s !important; }
    .stTextArea textarea:focus { border-color: #3b82f6 !important; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1) !important; outline: none !important; }
    .stTextArea label { color: #000000 !important; font-weight: 600 !important; font-size: 14px !important; }
    
    .stSelectbox label { color: #000000 !important; font-weight: 600 !important; font-size: 14px !important; }
    [data-baseweb="select"] { background: #FFFFFF !important; }
    [data-baseweb="select"] > div { background: #FFFFFF !important; border: 2px solid #e5e7eb !important; color: #000000 !important; }
    [data-baseweb="popover"] { background: #FFFFFF !important; }
    [data-baseweb="menu"] { background: #FFFFFF !important; }
    [role="option"] { background: #FFFFFF !important; color: #000000 !important; }
    [role="option"]:hover { background: #f3f4f6 !important; color: #000000 !important; }
    ul[role="listbox"] { background: #FFFFFF !important; border: 2px solid #e5e7eb !important; border-radius: 12px !important; }
    ul[role="listbox"] li { background: #FFFFFF !important; color: #000000 !important; padding: 0.75rem 1rem !important; }
    ul[role="listbox"] li:hover { background: #f3f4f6 !important; }
    
    .stNumberInput input { border: 2px solid #e5e7eb !important; border-radius: 12px !important; padding: 0.875rem 1rem !important; font-size: 15px !important; color: #000000 !important; background: #FFFFFF !important; }
    .stNumberInput label { color: #000000 !important; font-weight: 600 !important; font-size: 14px !important; }
    
    .stFileUploader { background: #FFFFFF !important; border: 2px solid #e5e7eb !important; border-radius: 12px !important; padding: 1rem !important; }
    .stFileUploader label { color: #000000 !important; font-weight: 600 !important; font-size: 14px !important; }
    
    .stCheckbox label { color: #000000 !important; font-size: 15px !important; }
    .stCheckbox span { color: #000000 !important; }
    
    .sidebar-title { padding: 0 1.5rem; margin-bottom: 2rem; }
    .sidebar-title h2 { font-size: 22px; font-weight: 700; color: #000000 !important; font-family: 'Space Grotesk', sans-serif; }
    
    .user-profile {
        background: #FFFFFF; padding: 1rem 1.25rem; border-radius: 14px;
        box-shadow: 0 4px 16px rgba(0,0,0,0.1); border: 2px solid #e5e7eb;
        display: flex; align-items: center; gap: 1rem; margin-top: 1rem; margin-bottom: 1rem;
    }
    .profile-avatar {
        width: 48px; height: 48px; border-radius: 12px;
        background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%);
        display: flex; align-items: center; justify-content: center;
        font-size: 22px; box-shadow: 0 2px 8px rgba(139, 92, 246, 0.3);
    }
    .profile-info h4 { font-weight: 600; color: #000000 !important; font-size: 14px; margin-bottom: 2px; }
    .profile-info p { font-size: 12px; color: #6b7280 !important; }
    
    .stSuccess { background: #FFFFFF !important; border: 2px solid #10b981 !important; border-radius: 12px !important; padding: 1rem 1.25rem !important; color: #000000 !important; }
    .stError { background: #FFFFFF !important; border: 2px solid #ef4444 !important; border-radius: 12px !important; padding: 1rem 1.25rem !important; color: #000000 !important; }
    .stInfo { background: #FFFFFF !important; border: 2px solid #3b82f6 !important; border-radius: 12px !important; padding: 1rem 1.25rem !important; color: #000000 !important; }
    .stWarning { background: #FFFFFF !important; border: 2px solid #f59e0b !important; border-radius: 12px !important; padding: 1rem 1.25rem !important; color: #000000 !important; }
    
    button[data-baseweb="tab"] { background: #FFFFFF !important; color: #000000 !important; border-radius: 10px !important; padding: 10px 24px !important; font-weight: 500 !important; border: 2px solid #e5e7eb !important; }
    button[data-baseweb="tab"][aria-selected="true"] { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%) !important; color: #FFFFFF !important; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3) !important; border: none !important; }
    button[data-baseweb="tab"] p { color: #000000 !important; }
    button[data-baseweb="tab"][aria-selected="true"] p { color: #FFFFFF !important; }
    
    [data-testid="stMetricValue"] { color: #000000 !important; font-size: 28px !important; font-weight: 700 !important; }
    [data-testid="stMetricLabel"] { color: #000000 !important; font-weight: 600 !important; }
    
    .stMarkdown h1, .stMarkdown h2, .stMarkdown h3, .stMarkdown h4 { color: #000000 !important; }
...
if 'sidebar_menu' not in st.session_state:
    st.session_state.sidebar_menu = 'AI Assistant'
...
def main_app():
    ...
    if st.session_state.current_page == "AI Assistant":
        ...
        if send_btn and user_input:
            ...
            with st.spinner("🤖 BankBot is thinking..."):
                bot_response, response_type = get_bot_response(user_input, st.session_state.chat_messages)
            st.session_state.chat_messages.append({
                "role": "bot",
                "content": bot_response,
                "timestamp": datetime.now().strftime("%I:%M %p"),
    ...
    # ── Chat History page ──────────────────────────────────────────────────────
        ...
        if st.session_state.chat_sessions:
            ...
            sorted_sessions = sorted(st.session_state.chat_sessions, key=lambda x: x['created_at'], reverse=True)
            for idx, session in enumerate(sorted_sessions):
                    ...
                    <div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; margin-bottom: 1rem; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.75rem;">
                            <div style="flex: 1;">
                                ...
                                <p style="color: #6b7280; margin: 0; font-size: 13px;">📅 {session['created_at']} • 💬 {session['message_count']} messages</p>
                            </div>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
                ...
                with col2:
                    if st.button(f"📥 Export", key=f"export_{session['id']}", use_container_width=True):
                        ...
                        st.download_button(label="💾 Download", data=export_text, file_name=f"conversation_{session['id']}.txt", mime="text/plain", key=f"download_{session['id']}")
                with col3:
    ...
    # ── Dashboard page ─────────────────────────────────────────────────────────
        ...
        with col1:
            st.metric(label="💬 Total Conversations", value=user_messages, delta="Active")
        with col2:
            st.metric(label="📨 Total Messages", value=total_messages, delta=f"+{total_messages}")
        with col3:
            st.metric(label="✅ Queries Resolved", value=bot_messages, delta="100%")
        
        st.markdown("---")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("### 🌟 Popular Topics")
            topics = ["💳 Account Services", "🔒 Security & PIN Management", "💰 Loans & Interest Rates", "📱 Digital Banking", "💸 Transfers & Payments", "📞 Customer Support"]
            for topic in topics:
                st.markdown(f"- {topic}")
        with col2:
    ...
    # ── Accounts page ──────────────────────────────────────────────────────────
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown("""<div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <p style="color: #6b7280; font-size: 13px;">Available Balance</p></div>""", unsafe_allow_html=True)
        with col2:
            st.markdown("""<div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
        ...
        with col3:
            st.markdown("""<div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <p style="color: #6b7280; font-size: 13px;">Maturity: Dec 2026</p></div>""", unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### 🔧 Account Actions")
        cols = st.columns(4)
        account_actions = [("📊", "View Statement"), ("📥", "Download Passbook"), ("🔄", "Recent Transactions"), ("⚙️", "Account Settings")]
            ...
            st.markdown(f"""
                <div style="background: white; padding: 1rem 1.5rem; margin: 0.5rem 0; border-radius: 12px; border: 2px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center;">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        ...
                        <div><p style="color: #000000; font-weight: 600; margin: 0;">{desc}</p><p style="color: #6b7280; font-size: 13px; margin: 0;">{date}</p></div>
                    </div>
    ...
    # ── Transfer page ──────────────────────────────────────────────────────────
        ...
        for idx, (emoji, method, desc, color) in enumerate(transfer_methods):
            with cols[idx]:
                st.markdown(f"""<div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; text-align: center;">
                    <div style="font-size: 40px; margin-bottom: 0.5rem;">{emoji}</div>
                    <h4 style="color: #000000; margin: 0.5rem 0;">{method}</h4>
                    <p style="color: {color}; font-size: 13px; margin: 0;">{desc}</p></div>""", unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### 📤 Quick Transfer")
        col1, col2 = st.columns(2)
        with col1:
            from_account = st.selectbox("From Account", ["Savings Account (XXXX 1234) - ₹1,24,500", "Current Account (XXXX 5678) - ₹45,750"])
            beneficiary = st.selectbox("To Beneficiary", ["Add New Beneficiary", "John Smith - 1234567890", "Jane Doe - 9876543210", "ABC Company - 5555555555"])
            amount = st.number_input("Amount (₹)", min_value=1, max_value=100000, value=1000, step=100)
        with col2:
            transfer_type = st.selectbox("Transfer Type", ["UPI (Instant & Free)", "NEFT (Free)", "RTGS (₹25-₹55)", "IMPS (₹5-₹15)"])
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("""<div style="background: white; padding: 1.5rem; border-radius: 12px; border: 2px solid #e5e7eb;">
                ...
                <p style="color: #000000;">• Monthly Limit: ₹30,00,000</p></div>""", unsafe_allow_html=True)
        with col2:
            st.markdown("""<div style="background: white; padding: 1.5rem; border-radius: 12px; border: 2px solid #e5e7eb;">
                ...
                <p style="color: #000000;">• IMPS: ₹5,00,000 per transaction</p></div>""", unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### 👥 Recent Beneficiaries")
        beneficiaries = [("👤", "John Smith", "9876543210", "₹5,000"), ("👤", "Jane Doe", "8765432109", "₹2,500"), ("🏢", "ABC Company", "7654321098", "₹10,000"), ("👤", "Bob Wilson", "6543210987", "₹1,200")]
        cols = st.columns(4)
    ...
    # ── Support page ───────────────────────────────────────────────────────────
        ...
        for idx, (emoji, method, detail, timing) in enumerate(contact_methods):
            with cols[idx]:
                st.markdown(f"""<div style="background: white; padding: 1.5rem; border-radius: 16px; border: 2px solid #e5e7eb; text-align: center;">
                    ...
                    <p style="color: #6b7280; font-size: 12px; margin: 0;">{timing}</p></div>""", unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### 🎫 Raise a Support Ticket")
        col1, col2 = st.columns(2)
        with col1:
            ticket_category = st.selectbox("Issue Category", ["Account Related", "Card Services", "Online Banking", "Loan Enquiry", "Complaint", "Other"])
            priority = st.selectbox("Priority", ["Low", "Medium", "High", "Urgent"])
        with col2:
            subject = st.text_input("Subject", placeholder="Brief description of your issue")
            contact_method = st.selectbox("Preferred Contact Method", ["Email", "Phone Call", "SMS", "WhatsApp"])
        description = st.text_area("Describe Your Issue", placeholder="Please provide detailed information about your issue...", height=150)
        ...
        faqs = [
            ("How do I reset my internet banking password?", "You can reset your password by clicking 'Forgot Password' on the login page. Enter your User ID and registered mobile number to receive an OTP. Use the OTP to create a new password."),
            ("What should I do if my debit card is lost or stolen?", "Immediately call our 24/7 helpline at 1800-XXX-XXXX to block your card. You can also use the 'Block Card' feature in our mobile app. A replacement card will be issued within 5-7 business days."),
            ("How can I update my mobile number?", "Visit your nearest branch with valid ID proof and a written request. Alternatively, if you have internet banking, you can update your mobile number through the profile settings after OTP verification."),
            ("What are the charges for cheque book?", "The first cheque book (25 leaves) is free. Additional cheque books cost ₹50 for 25 leaves. You can request a cheque book through internet banking, mobile app, or at any branch."),
            ("How long does NEFT transfer take?", "NEFT transfers are processed in hourly batches from 8 AM to 7 PM on working days. Typically, the beneficiary receives the amount within 2-3 hours. NEFT is free of charge for retail customers.")
        ]
    ...
    # ── Fallback ───────────────────────────────────────────────────────────────
        ...
        if st.button("← Back to AI Assistant", key="back_to_ai"):
            st.session_state.current_page = "AI Assistant"
            st.rerun()
